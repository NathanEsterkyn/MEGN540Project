/**
 * Stepper.h/c defines functions to create stepper objects and control stepper
 * motors
 */
...
/**
 * Function Initialize_Stepper initializes the stepper and creates a stepper
 * object that stores the current position. This function also initializes which
 * pins the stepper will be connected to - future dev
 * @param p_step Stepper object to populate
 * @param number_of_steps number of steps per revolution - stepper specific
 * @param motor_pin_1 identifies which 4 pins the H bridge is on based on AIN2
 * (MP1)
 */
